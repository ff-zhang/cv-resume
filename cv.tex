...
\section{Education}
  ...
  \resumeSubheadingThree
    {Honours Bachelor of Science, University of Toronto}{Sept. 2021 -- June 2025}
...
{Supervisor:\ Prof.\ Qizhen Zhang}{}
  \resumeItemListStart
  \resumeItem{Investigated the applicability of offloading data processing onto data processing units within data centers}
  % \resumeItem{Implemented the \emph{Monodepth2} depth estimation model in \textbf{C++} from scratch}
  \resumeItem{Parallelized the execution of \emph{Monodepth2} in \textbf{Python} and \textbf{C++} with the latter achieving linear performance scaling with the number of threads}
  \resumeItem{Worked on a team of 6 to implement a prefetcher with DPUs that intercepts network traffic to predict the data that will be requested by a tiered key-value store}
  \resumeItemListEnd
...
{ML Runtime Engineer}{\href{https://www.cerebras.net/}{Cerebras Systems}}{May 2024 -- Present}
  \resumeItemListStart
  \resumeItem{Implemented a runtime virtual memory system in \textbf{C++} with a team of $3$ which pre-emptively loads data before it is accessed, allowing off-chip memory to be used for the first time with a \textbf{10\%} performance penalty}
  ...
  \resumeItem{Enabled logging of all network operations and the ability to later replay the logged operations, decreasing the time to recreate stalls and timeouts by over \textbf{80\%}}
  % \resumeItem{Benchmarked multiple new server configurations under various expected workloads, informing current infrastructure designs}
  \resumeItemListEnd
...
\section{Projects}
  \resumeSubHeadingListStart
    ...
    \resumeItemListStart
      ...
      \resumeItem{Used \textbf{Python Optimal Transport} to compute various functions which transform the \emph{EMNIST} dataset of handwritten digits such that its distribution and priors match those of the \emph{MNIST} dataset}
      \resumeItem{Found that the accuracy of a fully-connected feed-forward classifier trained on the \emph{MNIST} dataset was improved from 17\% to \textbf{73\%} on the \emph{EMNIST} dataset}
    \resumeItemListEnd
    ...
    \resumeProjectHeading
      ...
      \resumeItemListStart
        ...
        \resumeItem{Achieved \textbf{92\%} accuracy on the \emph{MNIST} dataset of handwritten digits (within $2\%$ of the top classifier using SGD)}
